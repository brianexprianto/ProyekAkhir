...
<div class="col-md-12">
 ...
 <!-- form start -->
            <form class="form-horizontal" method="post" action="report_buy.php" >
            <div class="row">
...
if($bulan == null || $search == "" ){
        ...
        $sqla="SELECT no, COUNT( * ) AS totaldata FROM stok_masuk_daftar  WHERE tglmasuk BETWEEN '" . $dr . "' AND  '" . $sam . "' ";
      }
...
?>
          ...
          <span class="no-print pull-right" id="no-print"> <?php echo 'Total Penjualan pada <b>'.$namabulan.'</b> '.$tahun.' sejumlah <b>Rp '.number_format($totalinv, $decimal, $a_decimal, $thousand).',-</b>'; ?></span>
        <?php } ?>
    ...
    if(isset($sam)){
    $sql    = "select s.*,t.* from stok_masuk_daftar s join stok_masuk t on s.nota = t.nota  WHERE tglmasuk BETWEEN '" . $dr . "' AND  '" . $sam . "' order by s.no desc";
  } else {
      $sql    = "select s.*,t.* from stok_masuk_daftar s join stok_masuk t on s.nota = t.nota order by s.no desc";
  }
...
?>
    ...
    if ($bulan != null || $bulan != "") {
        ...
        $query1="SELECT s.*,t.* from stok_masuk_daftar s join stok_masuk t on s.nota = t.nota  where YEAR(s.tglmasuk) like '%$tahun%' AND MONTH(s.tglmasuk) like '%$bulan%' order by s.no limit $rpp";
...
<tr>
  ...
  <td><?php  echo mysqli_real_escape_string($conn, $fill['tglmasuk']); ?></td>
  <?php
$nota = $fill['nota'];
...
$jmljual=$row1['tot_jual'];
  ...
  <td><?php  echo mysqli_real_escape_string($conn,'Rp '. number_format($fill['biaya_pesan'])); ?></td>
