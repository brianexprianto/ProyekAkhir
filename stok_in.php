...
if(isset($_GET['sku'])) {
   $sku = mysqli_real_escape_string($conn, $_GET["sku"]);
    $sql1= "SELECT * FROM barang where sku='$sku'";
    ...
    if(isset($_POST["masuk"])){
       if($_SERVER["REQUEST_METHOD"] == "POST"){
               $nota = mysqli_real_escape_string($conn, $_POST["nota"]);
                $kode = mysqli_real_escape_string($conn, $_POST["kode"]);
                 $nama = mysqli_real_escape_string($conn, $_POST["nama"]);
                  ...
                  $biaya_pesan = mysqli_real_escape_string($conn, $_POST["biaya_pesan"]);
                  $lead_time = mysqli_real_escape_string($conn, $_POST["lead_time"]);
                  $tgl= date('Y-m-d');
         ...
         //            $upd=mysqli_query($conn,$sql3);
                    ...
                    $sql2 = "insert into stok_masuk_daftar values( '$nota','$kode','$nama','$jumlah','$biaya_pesan','$lead_time','$tgl','')";
...
<form method="get" action="">
   ...
   <div class="row" >
           <div class="form-group col-md-12 col-xs-12" >
              ...
              <?php
              error_reporting(E_ALL ^ (E_NOTICE | E_WARNING));
         $sql=mysqli_query($conn,"select *,barang.nama as nama, barang.kode as kode, barang.sku as sku, barang.eoq as eoq from barang");
        while ($row=mysqli_fetch_assoc($sql)){
          if ($barcode==$row['barcode'])
          echo "<option value='".$row['kode']."' nama='".$row['nama']."' kode='".$row['kode']."' stok='".$row['sisa']."' eoq='".$row['eoq']."' >".$row['sku']." | ".$row['nama']."</option>";
          else
          echo "<option value='".$row['kode']."' nama='".$row['nama']."' kode='".$row['kode']."' stok='".$row['sisa']."' eoq='".$row['eoq']."' >".$row['sku']." | ".$row['nama']."</option>";
        }
      ?>
...
<div class="row" >
           <div class="form-group col-md-12 col-xs-12" >
                  ...
                  <div class="col-sm-5">
                      <input type="text" class="form-control" id="jumlah" name="jumlah" value="<?php echo $eoq;?>" >
                  </div>
...
<div class="row" >
           <div class="form-group col-md-12 col-xs-12" >
                  <label for="barang" class="col-sm-2 control-label">Biaya Pesan:</label>
                  <div class="col-sm-5">
                      <input type="text" class="form-control" id="biaya_pesan" name="biaya_pesan" value="<?php echo $biaya_pesan;?>" >
                  </div>
...
<div class="row" >
           <div class="form-group col-md-12 col-xs-12" >
                  <label for="barang" class="col-sm-2 control-label">Lead Time:</label>
                  <div class="col-sm-5">
                      <input type="text" class="form-control" id="lead_time" name="lead_time" value="<?php echo $lead_time;?>" >
                  </div>
                   <div class="col-sm-5">
...
<form method="post" action="">
      ...
      ?>
          ...
          <div class="row">
           ...
           <td><?php  echo mysqli_real_escape_string($conn, $fill['biaya_pesan']); ?></td>
           <td><?php  echo mysqli_real_escape_string($conn, $fill['lead_time']); ?></td>
  ...
  var nama = $("#produk option:selected").attr("nama");
   var kode = $("#produk option:selected").attr("kode");
    var stok = $("#produk option:selected").attr("stok");
      var eoq = $("#produk option:selected").attr("eoq");
